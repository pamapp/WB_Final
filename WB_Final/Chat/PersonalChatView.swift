        ...
        case .favorite:
            fetchedBreeds.loadFavorites { data, error in
                if let favorites = data {
                    fetchedBreeds.favoriteBreeds = favorites
                    vm.messages += fetchedBreeds.getFavoriteBreedMessages().map { $0.message }
                }
            }
        default:
            fetchedBreeds.loadBreeds { breedMessages, error in
...
extension PersonalChatView {
    ...
    private func addFavorite(_ imageId: String) async -> Bool {
        await fetchedBreeds.addFavorite(imageId: imageId, subId: subId)
    }
